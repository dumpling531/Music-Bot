module.exports = {
    ...
    execute(client, message) {
        ...
        if (client.player.getQueue(message).paused) return message.channel.send(`${client.emotes.error} - 음악이 이미 일시 중지되었습니다`);

        client.player.pause(message);

        message.channel.send(`${client.emotes.success} - 노래 ${client.player.getQueue(message).playing.title} 일시중지`);
    },
};